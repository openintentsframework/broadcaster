...
contract DeployArbitrumParentToChild is DeployBase {
    function run() public {
        address outbox = vm.envAddress("OUTBOX");
        uint256 rootsSlot = vm.envUint("ROOTS_SLOT");
        ...
        bytes memory proverCreationCode =
            abi.encodePacked(type(ParentToChildProver).creationCode, abi.encode(outbox, rootsSlot, homeChainId));
