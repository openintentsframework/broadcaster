...
contract BroadcasterTest is Test {
    ...
    function encode(L2BlockHeader memory h) internal pure returns (bytes memory out) {
        RLP.Encoder memory enc = RLP.encoder().push(h.sendRoot).push(h.sendCount).push(h.l1BlockNumber)
            // .push(h.parentHash)
            // .push(h.sha3Uncles)
            // .push(h.miner)
            // .push(h.stateRoot)
    ...
    function test_verifyStorageSlot_invalid_proof() public {
        ...
        bytes memory input =
            abi.encode(encode(blockHeader), testAccount, testSlot, invalidAccountProof, invalidStorageProof);
    ...
    function test_verifyStorageSlot_max_slot_value() public {
        ...
        bytes memory input = abi.encode(encode(blockHeader), testAccount, maxSlot, accountProof, storageProof);
    ...
    function test_verifyStorageSlot_zero_address() public {
        ...
        bytes memory input = abi.encode(encode(blockHeader), zeroAccount, testSlot, accountProof, storageProof);
    ...
    function test_broadcast_and_verify_message() public {
        ...
        bytes memory input =
            abi.encode(encode(blockHeader), broadcasterAccount, messageSlot, accountProof, storageProof);
    ...
    function test_verifyTargetBlockHash_different_block_numbers() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            try childToParentProver.verifyTargetBlockHash(keccak256(encode(l2BlockHeader)), input) returns (
                bytes32 result
            ) {
    ...
    function test_verifyStorageSlot_different_accounts() public {
        ...
        for (uint256 i = 0; i < testAccounts.length; i++) {
            bytes memory input = abi.encode(encode(blockHeader), testAccounts[i], testSlot, accountProof, storageProof);

            try childToParentProver.verifyStorageSlot(targetBlockHash, input) returns (
    ...
    function test_verifyStorageSlot_different_slots() public {
        ...
        for (uint256 i = 0; i < testSlots.length; i++) {
            bytes memory input = abi.encode(encode(blockHeader), testAccount, testSlots[i], accountProof, storageProof);
