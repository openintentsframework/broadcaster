...
contract BroadcasterTest is Test {
    ...
    function encode(L2BlockHeader memory h) internal pure returns (bytes memory out) {
        RLP.Encoder memory enc = RLP.encoder().
            ...
            // .push(h.requestsHash)
            push(h.sendRoot).push(h.sendCount).push(h.l1BlockNumber).push(1);
    ...
    function test_verifyTargetBlockHash_different_block_numbers() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            try childToParentProver.verifyTargetBlockHash(
                keccak256(encode(l2BlockHeader)), input
    ...
    function test_verifyStorageSlot_different_accounts() public {
        ...
        for (uint256 i = 0; i < testAccounts.length; i++) {
            ...
            try childToParentProver.verifyStorageSlot(
